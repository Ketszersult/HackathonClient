...
const float maxSpeedDist=150;
const float maxSpeed=250;
const float sin5 = 0.08716;
const float cos5 = 0.99619;
const float turnSpeedMult = 2;

/* Steering constants*/
const float steerLock=0.366519 ;
const float steerSensitivityOffset=80.0;
const float wheelSensitivityCoeff=1.5;
const float turnAngleMult=1.1;

/* ABS Filter Constants */
const float wheelRadius[4]={0.3306,0.3306,0.3276,0.3276};
    ...
    if (cs.trackPos < 1 && cs.trackPos > -1)
    {
        ...
        else
        {
            ...
            if(rxSensor>sxSensor)
            {
                ...
                targetSpeed = turnSpeedMult * maxSpeed*(cSensor*sinAngle/maxSpeedDist);
            }
...
float getSteer(structCarState cs)
{
    ...
    float targetAngle= turnAngleMult * (cs.angle-cs.trackPos*0.5);
...
float clutching(structCarState cs, float clutch)
{
  ...
  if(clutch > 0)
  {
	...
	clutch = fmin(maxClutch,(double)clutch);

	// if clutch is not at max value decrease it quite quickly
	if (clutch!=maxClutch)
	{
	  clutch -= delta;
	  clutch = fmax(0.0,(double)clutch);
	}
...
structCarControl DriveFunction(structCarState cs)
{
    ...
    if (stuck > stuckTime)
    {
        ...
        // Calculate clutching
        clutch = clutching(cs,clutch);

        // build a CarControl variable and return it
        structCarControl cc = {1.0,0.0,gear,steer,clutch};
        return cc;
    }
